...
# apiを公開するurl,ip   s3からアクセスされるから そのときに設定する
# origins = [
#     "http://localhost:8080",
#     "http://localhost:3000",
# ]
...
def storage_image(user_file:models.UserFile,db:Session = Depends(get_db),settings: config.Settings = Depends(get_settings)):
    ...
    # dbにrequest情報　保存　
    db_data = models.create_trial(db,file_path=entry_point,hashed_password=hashed_password,file_name=user_file.file_name)    
